  ...
  static load_serialized_synth(str) {
    ...
    if (isActiveCyberFX && (isBfxrSource || isZzfxSource)) {
      ...
      if (isBfxrSource) {
        ...
        res.attack  = (bfxr.attackTime  ** 2 * 100000)      / 44100; // Bfxr stage 0: ramp up
        res.decay   = (bfxr.sustainTime ** 2 * 100000)      / 44100; // Bfxr stage 1: flat with punch
        res.sustain = (bfxr.decayTime   ** 2 * 100000 + 10) / 44100; // Bfxr stage 2: fade to 0
        res.release = 0;
        res.frequency = 44100 * (bfxr.frequency_start ** 2 + 0.001) / 100;
        res.slide = bfxr.frequency_slide;
        res.deltaSlide = bfxr.frequency_acceleration;
        res.pitchJump = bfxr.pitch_jump_amount;
